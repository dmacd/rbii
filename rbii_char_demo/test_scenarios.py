...
def build_scenario_a_context_switching(data_directory: Path) -> scenario_description:
    ...
    for cycle_index in range(cycle_count):
        episode_text = shakespeare_text[shakespeare_offset : shakespeare_offset + episode_length]
        shakespeare_offset = (shakespeare_offset + episode_length) % max(1, len(shakespeare_text) - episode_length)
        _append_episode(episodes, stream_text_parts, "shakespeare", episode_text)

        episode_text = dr_seuss_text[dr_seuss_offset : dr_seuss_offset + episode_length]
        dr_seuss_offset = (dr_seuss_offset + episode_length) % max(1, len(dr_seuss_text) - episode_length)
        _append_episode(episodes, stream_text_parts, "dr_seuss", episode_text)

        episode_text = generate_numeric_progression_text(step_size=1, length=episode_length)
        _append_episode(episodes, stream_text_parts, "numeric_progression_step_1", episode_text)

        episode_text = paired_word_text[paired_word_offset : paired_word_offset + episode_length]
...
def build_scenario_c_rare_glimpses(data_directory: Path) -> scenario_description:
    ...
    for index in range(glimpse_count + 1):
        ...
        _append_episode(episodes, stream_text_parts, "shakespeare_background", base_text[start:end])

        if index < glimpse_count:
            glimpse = paired_word_text[paired_word_offset : paired_word_offset + glimpse_length]
