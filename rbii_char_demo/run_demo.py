...
from run_logging import (
  ScenarioStepLogger,
  ScenarioStepLoggerConfiguration,
  uniform_loss_bits_for_vocabulary,
)
...
def run_scenario_stream(
    ...
    scenario_name: str,
    scenario_logger: ScenarioStepLogger,
) -> ScenarioRunResult:
  uniform_loss_bits_per_character = uniform_loss_bits_for_vocabulary(
    character_vocabulary)
  per_step_algorithm_loss_bits: list[float] = []
  per_step_baseline_loss_bits: list[float] = []

  total_steps = len(character_indices)
  with tqdm(total=total_steps,
    ...
    for step_index, observed_character_index in enumerate(character_indices,
      ...
      loss_bits = float(system.last_loss_bits or uniform_loss_bits_per_character)
      per_step_algorithm_loss_bits.append(loss_bits)
      per_step_baseline_loss_bits.append(float(uniform_loss_bits_per_character))

      frozen_store_size = len(system.frozen_store.list_program_identifiers())
      progress_bar.set_postfix({
        "loss_bits": f"{loss_bits:.3f}",
        "frozen_store": frozen_store_size,
      }, refresh=False)
      progress_bar.update(1)

      scenario_logger.log_step(
        scenario_name=scenario_name,
        step_index=step_index,
        total_steps=total_steps,
        system=system,
        loss_bits=loss_bits,
        uniform_loss_bits_per_character=uniform_loss_bits_per_character,
      )
...
def run_scenario(
  ...
  scenario_logger.start_scenario(scenario.scenario_name)
  ...
  reference_loss_bits = compute_reference_loss_bits_for_scenario(scenario,
                                                                 vocabulary)
  cumulative_algorithm_loss = compute_cumulative_sum(
    run_result.per_step_algorithm_loss_bits)[-1]
  cumulative_baseline_loss = compute_cumulative_sum(
    run_result.per_step_baseline_loss_bits)[-1]
  ...
  plot_cumulative_compression_gain(
    scenario_outputs_directory /
    f"{scenario.scenario_name}_cumulative_compression_gain.png",
    per_step_algorithm_loss_bits=run_result.per_step_algorithm_loss_bits,
    per_step_baseline_loss_bits=run_result.per_step_baseline_loss_bits,
  ...
  if len(reacquisition_measurements) > 0:
    plot_reacquisition_delay(
      scenario_outputs_directory /
      f"{scenario.scenario_name}_reacquisition_delay.png",
      measurements=reacquisition_measurements,
      title=f"{scenario.scenario_name}: reacquisition delay",
    )
    plot_reacquisition_excess_loss(
      scenario_outputs_directory /
      f"{scenario.scenario_name}_reacquisition_excess_loss.png",
      measurements=reacquisition_measurements,
  ...
  (scenario_outputs_directory /
   f"{scenario.scenario_name}_metrics.json").write_text(
...
def main() -> None:
  ...
  scenario_logger = ScenarioStepLogger(
    ...
    configuration=ScenarioStepLoggerConfiguration(
      program_snapshot_interval_steps=100,
      prediction_pool_snapshot_interval_steps=250,
    ),
  )
  ...
  scenario_logger.write_dsl_documentation_file(
