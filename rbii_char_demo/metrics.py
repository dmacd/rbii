...
class character_bigram_reference_model:
    def __init__(self, character_vocabulary: character_vocabulary, training_indices: list[int], smoothing_alpha: float = 0.5) -> None:
        ...
        self.bigram_counts = numpy.zeros((vocabulary_size, vocabulary_size), dtype=numpy.int64)
        ...
        for previous_index, next_index in zip(training_indices[:-1], training_indices[1:]):
            self.bigram_counts[int(previous_index), int(next_index)] += 1
            self.unigram_counts[int(next_index)] += 1

    def predict_distribution(self, previous_character_index: int | None) -> numpy.ndarray:
...
def compute_reference_loss_bits_for_scenario(
    ...
    for episode in scenario.episodes:
        ...
        episode_indices = character_vocabulary.encode_text(scenario.stream_text[episode.start_index : episode.end_index])
        ...
        for offset, next_index in enumerate(episode_indices):
            ...
            reference_loss_bits[episode.start_index + offset] = -math.log2(probability_value)
...
def compute_cumulative_compression_gain_bits(
    ...
    cumulative_gain = []
    running_total = 0.0
    for algorithm_loss, baseline_loss in zip(per_step_algorithm_loss_bits, per_step_baseline_loss_bits):
...
def compute_reacquisition_measurements(
    ...
    for task_name, episodes in task_name_to_episodes.items():
        ...
        for return_index, episode in enumerate(episodes):
            episode_algorithm_losses = per_step_algorithm_loss_bits[episode.start_index : episode.end_index]
            episode_reference_losses = per_step_reference_loss_bits[episode.start_index : episode.end_index]
            ...
            for prefix_length in range(1, episode_length + 1):
                cumulative_regret += float(episode_algorithm_losses[prefix_length - 1]) - float(
