...
def plot_cumulative_loss(
    ...
    matplotlib_plotting.plot(time_steps, baseline_cumulative, label="uniform_baseline")

    if per_step_reference_loss_bits is not None:
        reference_cumulative = compute_cumulative_sum(per_step_reference_loss_bits)
        matplotlib_plotting.plot(time_steps, reference_cumulative, label="reference_bigram")
...
def plot_reacquisition_delay(
    ...
    task_names = sorted(set(measurement.task_name for measurement in measurements))
    for task_name in task_names:
        ...
        matplotlib_plotting.plot(return_indices, delays, marker="o", label=task_name)
...
def plot_reacquisition_excess_loss(
    ...
    for task_name in task_names:
        task_measurements = [m for m in measurements if m.task_name == task_name]
        return_indices = [m.return_index for m in task_measurements]
        excess_losses = [m.reacquisition_excess_loss_bits for m in task_measurements]
        matplotlib_plotting.plot(return_indices, excess_losses, marker="o", label=task_name)
