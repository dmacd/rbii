...
class MemoryMechanism(Protocol):
  ...
  def build_recall_key(self, memory_state: "MemoryStateProtocol") -> tuple[
    int, ...]: ...
...
class CharacterHistoryMemoryMechanism:
  ...
  def initialize(self,
    ...
    bigram_counts = numpy.zeros((vocabulary_size, vocabulary_size),
                                dtype=numpy.int64)
    trigram_counts: dict[tuple[int, int], numpy.ndarray] = {}
    recall_index: dict[tuple[int, ...], list[str]] = {}
  ...
  def update(self, memory_state: CharacterHistoryMemoryState,
    ...
    if len(memory_state.recent_character_indices) >= 1:
      ...
      memory_state.bigram_counts[
        previous_character_index, observed_character_index] += 1
    ...
    if len(memory_state.recent_character_indices) >= 2:
      ...
      if key not in memory_state.trigram_counts:
        memory_state.trigram_counts[key] = numpy.zeros(
          memory_state.character_vocabulary.size, dtype=numpy.int64)
  ...
  def build_prediction_features(
    ...
    uniform_distribution = numpy.ones(vocabulary_size,
    ...
    if len(memory_state.recent_character_indices) >= 1:
      ...
      bigram_distribution = self._counts_to_probability_distribution(
        bigram_counts, memory_state.smoothing_alpha)
    else:
    ...
    if len(memory_state.recent_character_indices) >= 2:
      ...
      if key in memory_state.trigram_counts:
        ...
        trigram_distribution = self._counts_to_probability_distribution(
          trigram_counts, memory_state.smoothing_alpha)
      else:
    ...
    if floor_value > 0.0:
      bigram_distribution = (
                                  1.0 - floor_value) * bigram_distribution + floor_value * uniform_distribution
      trigram_distribution = (
                                   1.0 - floor_value) * trigram_distribution + floor_value * uniform_distribution

    bigram_distribution = bigram_distribution / float(bigram_distribution.sum())
    trigram_distribution = trigram_distribution / float(
      trigram_distribution.sum())
  ...
  def build_recall_key(self, memory_state: CharacterHistoryMemoryState) -> \
  tuple[int, ...]:
  ...
  def record_frozen_program_identifier(
    ...
    # Move-to-front
    updated = [frozen_program_identifier] + [identifier for identifier in
                                             existing if
                                             identifier != frozen_program_identifier]
    memory_state.recall_index[recall_key] = updated[
      : self.maximum_program_identifiers_per_key]
